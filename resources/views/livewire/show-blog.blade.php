                                    ...
                                    <div class="col-md-6" data-aos="fade">
                                        <article class="blog-post">
                                            ...
                                            <div class="pt-4">
                                                ...
                                                <h2 class="h4"><a class="text-black" href="{{route('blogDetail', $article->id)}}" wire:navigate>{{$article->title}}</a></h2>
                                                {!! $article->content !!} <a href="{{route('blogDetail', $article->id)}}" wire:navigate class="text-primary fw-bold" aria-label="Read the full article by clicking here">Read More</a>
                                            </div>
                                        </article>
                                    </div>    
                                    @php
                        ...
                        <ul class="list-unstyled widget-list">
                            @if($categories->isNotEmpty())
                                ...
                                @foreach($categories as $cat)    
                                    <li><a wire:navigate href="{{ route('blog'). '?cat='.$cat->slug}}">{{$cat->name}} <small class="ml-auto">(1)</small></a>
                                    </li>
                                    @php
                                        $i++;
                                    @endphp
                            ...
                            @foreach($latestArticles as $la)    
                                <ul class="list-unstyled widget-list">
                                    <li class="d-flex widget-post align-items-center">
                                        <a class="text-black" href="{{route('blogDetail', $la->id)}}" wire:navigate>
                                            <div class="widget-post-image flex-shrink-0 me-3">
                                                @if($la->image != '')
                                                    <img class="rounded" loading="lazy" decoding="async" src="{{ asset('storage/'.$la->image)}}" alt="Post Thumbnail">
                                                @endif       
                                            </div>
                                        </a>
                                        <div class="flex-grow-1">
                                            <h5 class="h6 mb-0"><a class="text-black" href="{{route('blogDetail', $la->id)}}" wire:navigate>{{$la->title}}</a></h5>
                                            <small>{{ \Carbon\Carbon::parse($la->created_at)->format('d M, Y') }}</small>
                                        </div>
                                    </li>
                                </ul>
                                @php
